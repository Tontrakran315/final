                ...
                <div class="w-full px-4 md:w-12/12">
                  ...
                  <label class="block my-3 text-gray-700 text-md"
                    ...
                    <span class="text-red-500 text-xs"
                      >(ไฟล์ที่อัปโหลดต้องมีขนาดไม่เกิน 8MB)</span
                    ></label
                  >
                  <input
                    ref="fileupload"
...
export default {
  ...
  methods: {
    ...
    async getMaterial() {
      try {
        let res = await http.get(`material/show/update/${this.id}`);
        if (res) {
    ...
    onSubmit() {
      ...
      if (!this.v$.$error) {
        ...
        //? Send data to API
        http
          .post(`material/update/${this.id}`, data)
          .then((res) => {
            if (res.data.success) {
