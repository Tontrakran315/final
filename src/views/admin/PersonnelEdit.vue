...
export default {
  ...
  methods: {
    //? Personnel
    async getPersonnel() {
      let personnel = await http.get(
        `personnel/show/update/${this.personnelId}`
      );
      if (personnel) {
        let data = personnel.data.data;
        this.citizenId = data.citizen_id;
        this.titleName = data.name_title;
        let name_th = data.name_th.split(" ");
        let name_en = data.name_en.split(" ");
        this.firstName = name_th[0];
        this.lastName = name_th[1];
        this.fistNameEn = name_en[0];
        this.lastNameEn = name_en[1];
        this.personnelType = data.personnel_type;
        this.academicType = data.academic_type;
        this.academicPosition = data.position_academic;
        this.managerPosition = data.position_manager;
        this.education = data.education;
        this.email = data.email;
        this.phoneNumber = data.tel_number;
        this.statusWork = data.work_status;
    ...
    onSubmit() {
      ...
      if (!this.v$.$error) {
        ...
        //? Call API
        http
          .post(`personnel/update/${this.personnelId}`, data)
          .then((res) => {
            if (res.data.success) {
