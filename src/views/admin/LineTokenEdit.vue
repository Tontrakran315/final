        ...
        <div class="container px-4 mx-auto">
          <div class="px-6">
            <div class="mt-4 text-right">
              <router-link to="/admin/linetokenshow">
                <i
            ...
            <div class="text-center">
              <h1 class="py-6 text-3xl font-bold">อัปเดตข้อมูล Line Token</h1>
            </div>
...
export default {
  ...
  methods: {
    //? Get line token data
    async getLineTokenData() {
      try {
        const response = await http.get(`personnel/line-token/show/${this.id}`);
        if (response?.data?.success) {
          let data = response.data.data;
          this.personnel = data?.personnel_id;
          this.role = data?.token_role;
          this.token = data?.line_token;
        } else {
    ...
    async createLineToken(data) {
      try {
        const response = await http.post(
          `personnel/line-token/update/${this.id}`,
          data
        );
